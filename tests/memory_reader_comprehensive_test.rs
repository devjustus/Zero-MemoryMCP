...
fn test_basic_reader_all_types() {
    ...
    assert_eq!(reader.read::<u8>(Address::new(&u8_val as *const _ as usize)).unwrap(), u8_val);
    assert_eq!(reader.read::<u16>(Address::new(&u16_val as *const _ as usize)).unwrap(), u16_val);
    assert_eq!(reader.read::<u32>(Address::new(&u32_val as *const _ as usize)).unwrap(), u32_val);
    assert_eq!(reader.read::<u64>(Address::new(&u64_val as *const _ as usize)).unwrap(), u64_val);
    assert_eq!(reader.read::<i8>(Address::new(&i8_val as *const _ as usize)).unwrap(), i8_val);
    assert_eq!(reader.read::<i16>(Address::new(&i16_val as *const _ as usize)).unwrap(), i16_val);
    assert_eq!(reader.read::<i32>(Address::new(&i32_val as *const _ as usize)).unwrap(), i32_val);
    assert_eq!(reader.read::<i64>(Address::new(&i64_val as *const _ as usize)).unwrap(), i64_val);
    
    let f32_result = reader.read::<f32>(Address::new(&f32_val as *const _ as usize)).unwrap();
    assert!((f32_result - f32_val).abs() < 0.001);
    
    let f64_result = reader.read::<f64>(Address::new(&f64_val as *const _ as usize)).unwrap();
...
fn test_basic_reader_strings() {
    ...
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), "This is a much longer string for testing purposes");
...
fn test_safe_reader_all_value_types() {
    ...
    assert!(matches!(
        reader.read_value(Address::new(&u8_val as *const _ as usize), ValueType::U8).unwrap(),
        MemoryValue::U8(200)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&u16_val as *const _ as usize), ValueType::U16).unwrap(),
        MemoryValue::U16(50000)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&u32_val as *const _ as usize), ValueType::U32).unwrap(),
        MemoryValue::U32(3000000000)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&u64_val as *const _ as usize), ValueType::U64).unwrap(),
        MemoryValue::U64(9000000000000000000)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&i8_val as *const _ as usize), ValueType::I8).unwrap(),
        MemoryValue::I8(-100)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&i16_val as *const _ as usize), ValueType::I16).unwrap(),
        MemoryValue::I16(-20000)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&i32_val as *const _ as usize), ValueType::I32).unwrap(),
        MemoryValue::I32(-1500000000)
    ));
    
    assert!(matches!(
        reader.read_value(Address::new(&i64_val as *const _ as usize), ValueType::I64).unwrap(),
        MemoryValue::I64(-8000000000000000000)
    ));
    
    if let MemoryValue::F32(val) = reader.read_value(Address::new(&f32_val as *const _ as usize), ValueType::F32).unwrap() {
        assert!((val - 999.999).abs() < 0.01);
    } else {
        panic!("Expected F32");
    }
    
    if let MemoryValue::F64(val) = reader.read_value(Address::new(&f64_val as *const _ as usize), ValueType::F64).unwrap() {
        assert!((val - 999999.999999).abs() < 0.01);
    } else {
        panic!("Expected F64");
    }
    
    if let MemoryValue::String(val) = reader.read_value(Address::new(string_val.as_ptr() as usize), ValueType::String).unwrap() {
    ...
    if let MemoryValue::Bytes(val) = reader.read_value(Address::new(&bytes_data as *const _ as usize), ValueType::Bytes).unwrap() {
...
fn test_unified_reader_all_operations() {
    ...
    assert!(result.is_ok());
    assert!(matches!(result.unwrap(), MemoryValue::U64(0xDEADBEEFCAFEBABE)));
